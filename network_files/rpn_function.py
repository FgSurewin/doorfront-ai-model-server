...
class AnchorsGenerator(nn.Module):
    ...
    def __init__(self, sizes=(128, 256, 512), aspect_ratios=(0.5, 1.0, 2.0), multilevel=False):
    ...
    def generate_adaptive_anchors(self, scales, aspect_ratios, dtype=torch.float32, device=torch.device("cpu")):
        # type: (List[int], List[float], torch.dtype, torch.device) -> Tensor
        """
        compute anchor sizes
        Arguments:
        ...
        # if len(scales) != len(aspect_ratios):
        #     print("size of scales and Aspect ratios don't match!")
        #     assert len(scales) == len(aspect_ratios)
        # else:
        # for idx, scale in scales:
        scale = torch.as_tensor(scales, dtype=dtype, device=device)
        aspect_ratio = torch.as_tensor(aspect_ratios, dtype=dtype, device=device)
        ...
        return base_anchor.round()  # round 四舍五入
    ...
    def set_cell_anchors(self, dtype, device):
        ...
        if not self.multilevel:
            ...
            cell_anchors = [
                self.generate_adaptive_anchors(sizes, aspect_ratios, dtype, device)
                for sizes, aspect_ratios in zip(self.sizes, self.aspect_ratios)]
            cell_anchors = torch.cat(cell_anchors)
    ...
    def num_anchors_per_location_for_adaptive_anchors(self):
        # 计算每个预测特征层上每个滑动窗口的预测目标数
        anchors_per_location = 0
        for idx, scale in enumerate(self.sizes):
            anchors = len(scale)*len(self.aspect_ratios[idx])
            anchors_per_location += anchors
        return anchors_per_location
