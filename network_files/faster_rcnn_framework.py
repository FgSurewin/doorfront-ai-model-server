...
from .gcn_framework import GCNnet
...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None, adj_file=None, inp=None,
                 # transform parameter
                 ...
                 # Box parameters
                 box_roi_pool=None, box_head=None, box_predictor=None, gcn_net=None,
                 # 移除低目标概率      fast rcnn中进行nms处理的阈值   对预测结果根据score排序取前100个目标
                 box_score_thresh=0.0, box_nms_thresh=0.5, box_detections_per_img=100,
                 box_fg_iou_thresh=0.5, box_bg_iou_thresh=0.5,   # fast rcnn计算误差时，采集正负样本设置的阈值
                 box_batch_size_per_image=512, box_positive_fraction=0.25,  # fast rcnn计算误差时采样的样本数，以及正样本占所有样本的比例
                 bbox_reg_weights=None, multilevel_anchors=False):
        ...
        # 生成RPN通过滑动窗口预测网络部分
        if rpn_head is None:
            if not multilevel_anchors:
                rpn_head = RPNHead(
                    out_channels, rpn_anchor_generator.num_anchors_per_location()[0]
                )
            else:
                rpn_head = RPNHead(
                    out_channels, rpn_anchor_generator.num_anchors_per_location_for_adaptive_anchors()
                )
        ...
        # add gcn semantic reasoning for classification
        if gcn_net is None:
            gcn_net = GCNnet(num_classes, adj_file, in_channel=300, t=0.4)

        # 将roi pooling, box_head以及box_predictor结合在一起
        roi_heads = RoIHeads(
            # box
            box_roi_pool, box_head, box_predictor, gcn_net,
            box_fg_iou_thresh, box_bg_iou_thresh,  # 0.5  0.5
            box_batch_size_per_image, box_positive_fraction,  # 512  0.25
            bbox_reg_weights,
            box_score_thresh, box_nms_thresh, box_detections_per_img, inp)  # 0.05  0.5  100
